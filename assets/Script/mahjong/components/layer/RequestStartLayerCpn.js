...
cc.Class({
    ...
    init : function(data){
        ...
        if(comfirmUidList.length == cc.mj.gameData.getPlayerDataMgr().getPlayerNumber() ){
            this.showStartSucBox(comfirmUidList);
            cc.mj.gameData.changeSeatNum(comfirmUidList.length);

        }else if(isConfirm == false  || second <=0){
            this.showStartFailBox(choiceUid);
        }
    },
    ...
    onClickBtnChoice : function(target,data){
        ...
        this.isMeConfirm = true;
        this.isMeConfirm ? this.setBtnVisible(false) : this.setBtnVisible(true);
        tempClient.CommonUidConfirmStart(choice);
    },

    showStartFailBox : function(uid){
        this.closeMyself();
        if(uid){
    ...
    showStartSucBox : function(uidList){
        this.closeMyself();
